...
// Создаём подключение к БД с выводом всех SQL-запросов в консоль
...
// Проверяем, есть ли уже ПВЗ в базе
const pvzCount = db.prepare("SELECT COUNT(*) as cnt FROM pvz").get().cnt;

// Если ПВЗ нет — добавляем начальные данные
if (pvzCount === 0) {
    // Подготавливаем запрос для вставки ПВЗ
    const insertPvz = db.prepare(`
        INSERT INTO pvz (id, address, city) VALUES (?, ?, ?)
    `);

    // Список ПВЗ для заполнения: [id, адрес, город]
    const pvzList = [
    ...
    // Создаём транзакцию для атомарной вставки всех ПВЗ
...
// Экспортируем подключение к БД для использования в других файлах
module.exports = db;